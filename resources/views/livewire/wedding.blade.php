<div>
    <div class="sm:ml-5 md:ml-10">
        @include('layout.popup')
    </div>
    <div class="container-fluid">
        <div class="container mt-5 justify-content-center;">
                    <div class="text-center">
                        <strong class="text-center mb-4">“Portanto deixará o homem a seu pai e a sua mãe, e unir-se-á a 
                        sua mulher, e serão uma só carne.” – Gênesis 2.24</strong><br>
                         <span class="text-center mb-4">Com a benção de Deus e nossos Pais :</span>
                            <div class="row text-center">
                                <div class="col  mt-4">
                                    <p>Nelson Roque da Costa</p>
                                    <p>Catarina Peres da Costa</p>
                                </div>
                                 <div class="col mt-4">
                                    <p>Antônio Ramos <span class="material-symbols-outlined"> favorite </span></p>
                                    <p>Benedita de Sousa Ramos</p>
                                 </div>
                            </div>
                         
                         <h2 class="text-center">Valdinéia & Alexandre</h2>
                         <p class="">
                            Convidam para o seu casamento religioso
                            dia 29 de Abril de 2023 as 11:00 hs,
                            a cerimônia será realizada na Igreja Matriz de São Lourenço Mártir,
                            Praça Frei Egídio de Assis 2 São Lourenço MG.
                         </p>
                         <span class="text">
                            "Um casamento perfeito é feito de duas pessoas
                            imperfeitas que se recusam a desistir um do outro".
                         </span>
                    </div>
                    <div class="text-center mt-2">
                        <a href="{{ route('lovestore')}}">love story</a>
                    </div>
                    
        </div>
    </div>
 
</div>
